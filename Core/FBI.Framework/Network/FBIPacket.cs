...
namespace FBI.Framework.Network
{
	...
	/// </summary>
	public class FBIPacket : IDisposable
	{
		...
		/// <summary>
		/// Initializes a new instance of the FBIPacket class.
		/// </summary>
		/// <param name='net_message'>
		/// Net message.
		/// </param>
		public FBIPacket(string net_message)
		{
			_netmsg = net_message;
		...
		/// <summary>
		/// Releases all resource used by the FBIPacket object.
		/// </summary>
		/// <remarks>
		/// Call <see cref="Dispose"/> when you are finished using the FBIPacket. The
		/// <see cref="Dispose"/> method leaves the FBIPacket in an unusable state. After calling
		/// <see cref="Dispose"/>, you must release all references to the FBIPacket so the
		/// garbage collector can reclaim the memory that the FBIPacket was occupying.
		/// </remarks>
		public void Dispose()
		{
			read_position = 0;
