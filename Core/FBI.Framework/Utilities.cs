	...
	public sealed class Utilities
	{
		...
		public string GetUrl(string url, string args, string noencode)
		{
			lock(WriteLock)
			{
				...
				request.UserAgent = Consts.FBIUserAgent;
				request.Referer = Consts.FBIReferer;
		...
		public void DownloadFile(string url, string filename)
		{
			...
			{
				client.Headers.Add("referer", Consts.FBIReferer);
				client.Headers.Add("user-agent", Consts.FBIUserAgent);
		...
		public string GetRandomString()
		{
			...
			return path.Replace(FBIBase.Point.ToString(), string.Empty);
		}
...
//#endif
		...
		{
			return FBI.Framework.Config.Consts.FBIVersion;
		}
		...
		public string GetCpuId()
		{
			if(GetPlatformType() == PlatformType.Windows)
			{
				...
				return (from ManagementObject mo in mos.Get() select (Regex.Replace(Convert.ToString(mo["Name"]), @"\s+", FBIBase.Space.ToString()))).FirstOrDefault();
			}
			else if(GetPlatformType() == PlatformType.Linux)
			{
				...
				string cpu = (match.Groups["first"].ToString() + FBIBase.Space + match.Groups["second"].ToString());
				return cpu;
			}
		...
		{
			if(File.Exists(FBIBase.PidFile))
				File.Delete(FBIBase.PidFile);
		}
	}
}