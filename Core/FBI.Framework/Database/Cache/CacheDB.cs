...
namespace FBI.Framework.Database.Cache
{
	public sealed class CacheDB
	{
		...
		{
			var db = FBIBase.DManager.Query("SELECT Id, Language, Command, Text FROM localized_console_command");
			if(!db.IsNull())
			{
		...
		{
			var db = FBIBase.DManager.Query("SELECT Id, Language, Command, Text FROM localized_console_command_help");
			if(!db.IsNull())
			{
				foreach(DataRow row in db.Rows)
				{
		...
		{
			var db = FBIBase.DManager.Query("SELECT Id, Language, Command, Text FROM localized_console_warning");
			if(!db.IsNull())
			{
				foreach(DataRow row in db.Rows)
				{
		...
		{
			var db = FBIBase.DManager.Query("SELECT Id, ServerId, ServerName, Functions, Channel, Password, Enabled, Error, Language FROM channels");
			if(!db.IsNull())
			{
				foreach(DataRow row in db.Rows)
				{
