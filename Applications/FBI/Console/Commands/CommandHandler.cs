...
namespace FBI.Console.Commands
{
	...
	partial class CommandHandler : ConsoleLog
	{
		...
		protected void HandleConsoleLog()
		{
			...
			if(Info[1].ToLower() == FBIBase.On)
			{
				Log.Notice("Console", text[0]);
				ChangeLog(true);
			}
			else if(Info[1].ToLower() == FBIBase.Off)
			{
		...
		protected void HandleSys()
		{
			...
			Log.Notice("Console", text[6], FBIBase.timer.Uptime());
		}
		...
		protected void HandleConsoleToChannel()
		{
			...
			System.Console.Title = FBIBase.Title + " || Console Writing Channel: " + _servername + FBIBase.Colon + Info[1];
		}
		...
		protected void HandleOldServerToNewServer()
		{
			...
			System.Console.Title = FBIBase.Title + " || Console Writing Channel: " + Info[1] + FBIBase.Colon + _channel;
		}
		...
		protected void HandleFunction()
		{
			...
			if(Info[1].ToLower() == "channel")
			{
				...
				var db0 = FBIBase.DManager.QueryFirstRow("SELECT* FROM channels WHERE Channel = '{0}' And ServerName = '{1}'", Info[2].ToLower(), _servername);
				if(db0.IsNull())
				{
				...
				else if(status == FBIBase.On || status == FBIBase.Off)
				{
					if(Info.Length < 5)
					{
					...
					if(Info.Length >= 6)
					{
						...
						for(int i = 4; i < Info.Length; i++)
						{
							...
							if(sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[i], channel) && status == FBIBase.On)
							{
								onfunction += ", " + Info[i].ToLower();
								continue;
							}
							else if(!sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[i], channel) && status == FBIBase.Off)
							{
							...
							args += ", " + Info[i].ToLower();
							FBIBase.DManager.Update("channels", string.Format("Functions = '{0}'", sIrcBase.Networks[_servername].sChannelInfo.ChannelFunctions(Info[i].ToLower(), status, channel)), string.Format("Channel = '{0}' And ServerName = '{1}'", channel, _servername));
					...
					else
					{
						...
						if(sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[4], channel) && status == FBIBase.On)
						{
							Log.Warning("Console", sLManager.GetConsoleWarningText("FunctionAlreadyTurnedOn"));
							return;
						}
						else if(!sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[4], channel) && status == FBIBase.Off)
						{
						...
						FBIBase.DManager.Update("channels", string.Format("Functions = '{0}'", sIrcBase.Networks[_servername].sChannelInfo.ChannelFunctions(Info[4].ToLower(), status, channel)), string.Format("Channel = '{0}' And ServerName = '{1}'", channel, _servername));
						sIrcBase.Networks[_servername].sChannelInfo.ChannelFunctionsReload();
					}
				}
				else
			...
			else if(Info[1].ToLower() == "update")
			{
				...
				if(Info[2].ToLower() == "all")
				{
					...
					if(!db.IsNull())
					{
						foreach(DataRow row in db.Rows)
						{
							...
							FBIBase.DManager.Update("channels", string.Format("Functions = '{0}'", sUtilities.GetFunctionUpdate()), string.Format("Channel = '{0}' And ServerName = '{1}'", channel, _servername));
						}
				...
				else
				{
					...
					FBIBase.DManager.Update("channels", string.Format("Functions = '{0}'", sUtilities.GetFunctionUpdate()), string.Format("Channel = '{0}' And ServerName = '{1}'", Info[2].ToLower(), _servername));
					sIrcBase.Networks[_servername].sChannelInfo.ChannelFunctionsReload();
				}
			}
			...
			else
			{
				...
				if(Info[1].ToLower() == FBIBase.On || Info[1].ToLower() == FBIBase.Off)
				{
					if(Info.Length >= 4)
					{
						string args = string.Empty;
						...
						for(int i = 2; i < Info.Length; i++)
						{
							...
							if(sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[i]) && Info[1].ToLower() == FBIBase.On)
							{
								onfunction += ", " + Info[i].ToLower();
								continue;
							}
							else if(!sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[i]) && Info[1].ToLower() == FBIBase.Off)
							{
								offfunction += ", " + Info[i].ToLower();
								continue;
							}
					...
					else
					{
						...
						if(sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[2]) && Info[1].ToLower() == FBIBase.On)
						{
							Log.Warning("Console", sLManager.GetConsoleWarningText("FunctionAlreadyTurnedOn"));
							return;
						}
						else if(!sIrcBase.Networks[_servername].sChannelInfo.FSelect(Info[2]) && Info[1].ToLower() == FBIBase.Off)
						{
							Log.Warning("Console", sLManager.GetConsoleWarningText("FunctionAlreadyTurnedOff"));
							return;
						}
		...
		protected void HandleChannel()
		{
			...
			if(Info[1].ToLower() == "add")
			{
				...
				var db = FBIBase.DManager.QueryFirstRow("SELECT* FROM channels WHERE Channel = '{0}' And ServerName = '{1}'", channel, _servername);
				if(!db.IsNull())
				{
				...
				if(Info.Length == 4)
				{
					...
					FBIBase.DManager.Insert("`channels`(ServerId, ServerName, Channel, Password, Language)", ServerList.List[_servername].ServerId(), _servername, channel, pass, sLManager.Locale);
					FBIBase.DManager.Update("channels", "Enabled = 'true'", string.Format("Channel = '{0}' And ServerName = '{1}'", channel, _servername));
				}
				else
				{
					sIrcBase.Networks[_servername].sSender.Join(channel);
					FBIBase.DManager.Insert("`channels`(ServerId, ServerName, Channel, Password, Language)", ServerList.List[_servername].ServerId(), _servername, channel, string.Empty, sLManager.Locale);
					FBIBase.DManager.Update("channels", "Enabled = 'true'", string.Format("Channel = '{0}' And ServerName = '{1}'", channel, _servername));
				}
			...
			else if(Info[1].ToLower() == "remove")
			{
				...
				FBIBase.DManager.Delete("channels", string.Format("Channel = '{0}' And ServerName = '{1}'", channel, _servername));
			...
			else if(Info[1].ToLower() == "info")
			{
				...
				var db = FBIBase.DManager.Query("SELECT Channel, Enabled, Error FROM channels WHERE ServerName = '{0}'", _servername);
				if(!db.IsNull())
				{
			...
			else if(Info[1].ToLower() == "language")
			{
				...
				var db = FBIBase.DManager.QueryFirstRow("SELECT* FROM channels WHERE Channel = '{0}' And ServerName = '{1}'", Info[2].ToLower(), _servername);
				if(db.IsNull())
				{
					Log.Warning("Console", text[1]);
					return;
				...
				db = FBIBase.DManager.QueryFirstRow("SELECT Language FROM channels WHERE Channel = '{0}' And ServerName = '{1}'", Info[2].ToLower(), _servername);
				if(!db.IsNull())
				{
				...
				FBIBase.DManager.Update("channels", string.Format("Language = '{0}'", Info[3]), string.Format("Channel = '{0}' And ServerName = '{1}'", Info[2].ToLower(), _servername));
			...
			else if(Info[1].ToLower() == "password")
			{
				...
				if(Info[2].ToLower() == "add")
				{
					...
					var db = FBIBase.DManager.QueryFirstRow("SELECT* FROM channels WHERE Channel = '{0}' And ServerName = '{1}'", Info[3].ToLower(), _servername);
					if(db.IsNull())
					{
						Log.Warning("Console", text[0]);
						return;
					...
					db = FBIBase.DManager.QueryFirstRow("SELECT Password FROM channels WHERE Channel = '{0}' And ServerName = '{1}'", Info[3].ToLower(), _servername);
					if(!db.IsNull())
					{
					...
					FBIBase.DManager.Update("channels", string.Format("Password = '{0}'", Info[4]), string.Format("Channel = '{0}' And ServerName = '{1}'", Info[3].ToLower(), _servername));
				...
				else if(Info[2].ToLower() == "remove")
				{
					...
					FBIBase.DManager.Update("channels", "Password = ''", string.Format("Channel = '{0}' And ServerName = '{1}'", Info[3].ToLower(), _servername));
		...
		protected void HandleReload()
		{
			...
			{
				case "config":
					new Config(FBIConfig.ConfigDirectory, FBIConfig.ConfigFile, FBIConfig.ColorBindMode);
					sLConsole.Locale = LocalizationConfig.Locale;
					i = 1;
					break;
				case "cachedb":
					FBIBase.sCacheDB.ReLoad();
					i = 1;
					break;
			}
